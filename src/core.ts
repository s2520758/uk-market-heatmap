...
export type HeatmapNode = HeatmapLeaf | HeatmapGroup;
...
export type LayoutItem =
  | { type: "group"; depth: number; rect: Rect; name: string; headerHeight: number }
  | { type: "leaf"; depth: number; rect: Rect; node: HeatmapLeaf };
...
export type TileLabelMode = "full" | "ticker" | "none";

export interface TreemapAggregationEntry<T> {
  symbol: string;
  group: string;
  value: number;
  change: number;
...
export interface TreemapAggregate<T> {
  symbol: string;
  group: string;
  value: number;
  change: number;
...
export interface AggregatedTreemap<T> {
  data: HeatmapNode[];
 ...
 */
export function getTreemapDensity(stockCount: number): LayoutOptions {
...
export function getTileLabelMode(rect: Rect, symbol: string): TileLabelMode {
 ...
 */
export function squarify<T extends { area: number }>(items: T[], rect: Rect): (T & Rect)[] {
  const out: (T & Rect)[] = [];
  let i = 0;
  ...
  while (i < n) {
    ...
    // Greedily grow a row while the worst aspect ratio keeps improving.
    const start = i;
    ...
    // Lay the finished row along the shorter side, then shrink the rect.
    if (w >= h) {
      const colW = sum / h;
      let yy = y;
...
// Treat non-finite or non-positive sizes as zero so a malformed feed cannot
// corrupt the layout (it just drops the tile).
function safeSize(value: number): number {
  return Number.isFinite(value) && value > 0 ? value : 0;
}

function sumValue(node: HeatmapNode): number {
  if ("children" in node) return node.children.reduce((s, c) => s + sumValue(c), 0);
  return safeSize(node.value);
}

function groupHeaderHeight(rect: Rect, requested: number, pad: number, name: string): number {
...
export function layoutTree(
  ...
  const scored = nodes
    .map((node) => ({ node, value: sumValue(node) }))
    .filter((d) => d.value > 0)
...
function buildAggregatedTreemap<T>(
  entries: TreemapAggregationEntry<T>[],
  aggregatedSymbols: Set<string>,
): AggregatedTreemap<T> {
  ...
  const aggregates: TreemapAggregate<T>[] = [];

  for (const group of groupOrder) {
    const members = entries
      .filter((entry) => entry.group === group && aggregatedSymbols.has(entry.symbol))
      .sort((a, b) => b.value - a.value);
  ...
  const data = groupOrder
    .map((group) => {
      const individualLeaves: HeatmapLeaf[] = visible
      ...
      const aggregateLeaves: HeatmapLeaf[] = aggregates
        .filter((aggregate) => aggregate.group === group)
        .map((aggregate) => ({
 ...
 */
export function aggregateTinyLeaves<T>(
  entries: TreemapAggregationEntry<T>[],
  rect: Rect,
  opt: LayoutOptions,
): AggregatedTreemap<T> {
  ...
  for (let iteration = 0; iteration < entries.length; iteration += 1) {
    ...
    const newlyTiny = output
      .filter((item): item is Extract<LayoutItem, { type: "leaf" }> => item.type === "leaf")
      .filter((item) => !snapshot.aggregatesBySymbol.has(item.node.symbol))
      .filter(
        (item) => getTileLabelMode(item.rect, item.node.label ?? item.node.symbol) === "none",
      )
      .map((item) => item.node.symbol)
      .filter((symbol) => !aggregatedSymbols.has(symbol));
...
const NEUTRAL: [number, number, number] = [62, 68, 82];
const RED: [number, number, number] = [228, 60, 60];
const GREEN: [number, number, number] = [33, 191, 94];
...
export function changeColor(change: number, cap = 3): string {
